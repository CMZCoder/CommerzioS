...
// UI2 Pages
import UI2Home from "./home";
import UI2Search from "./search";
import UI2Service from "./service";
import UI2Profile from "./profile";
import UI2MyBookings from "./my-bookings";
import UI2Chat from "./chat";
import UI2Favorites from "./favorites";
import UI2Notifications from "./notifications";
import UI2BookService from "./book-service";
import UI2UserProfile from "./user-profile";
import UI2Referrals from "./referrals";

// Shared layout for UI2
function UI2Layout({ children }: { children: React.ReactNode }) {
  ...
  const navItems = [
    { href: "/ui2", label: "Home", icon: Home },
    { href: "/ui2/search", label: "Browse", icon: Search },
    { href: "/ui2/my-bookings", label: "Bookings", icon: Calendar },
    { href: "/ui2/chat", label: "Messages", icon: MessageCircle },
    { href: "/ui2/favorites", label: "Favorites", icon: Heart },
  ];

  const isActive = (href: string) => {
    if (href === "/ui2") return location === "/ui2";
    ...
    <div className="min-h-screen bg-slate-50">
        ...
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="hidden lg:flex items-center flex-1 max-w-xl mx-8">
              <Link href="/ui2/search" className="w-full">
              ...
              <Sheet open={mobileMenuOpen} onOpenChange={setMobileMenuOpen}>
                ...
                <SheetContent side="right" className="w-80 bg-slate-900 border-slate-800 text-white">
                  <div className="flex flex-col h-full">
                    ...
                    {/* Mobile Search */}
                    <Link href="/ui2/search" onClick={() => setMobileMenuOpen(false)}>
                    ...
                    <nav className="flex-1 space-y-1">
                      ...
                      {user && (
                        <>
                          <Link href="/ui2/notifications" onClick={() => setMobileMenuOpen(false)}>
                          ...
                          <Link href="/ui2/profile" onClick={() => setMobileMenuOpen(false)}>
                            <a className="flex items-center gap-3 px-4 py-3 rounded-xl text-slate-400 hover:text-white hover:bg-white/5">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 mb-8">
            ...
            <div>
              ...
              <ul className="space-y-2 text-sm text-slate-400">
                <li><Link href="/ui2/search?category=home"><a className="hover:text-white">Home Services</a></Link></li>
                <li><Link href="/ui2/search?category=design"><a className="hover:text-white">Design & Creative</a></Link></li>
                <li><Link href="/ui2/search?category=education"><a className="hover:text-white">Education</a></Link></li>
                <li><Link href="/ui2/search?category=business"><a className="hover:text-white">Business</a></Link></li>
              </ul>
            </div>
            <div>
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t shadow-lg z-40">
        <div className="flex items-center justify-around py-2">
          {navItems.map((item) => (
            <Link key={item.href} href={item.href}>
              <a className={`flex flex-col items-center gap-1 px-3 py-2 rounded-lg ${
                isActive(item.href) ? "text-violet-600" : "text-slate-500"
              }`}>
                <item.icon className="w-5 h-5" />
...
// Main UI2 Router
export default function UI2Router() {
  return (
    <UI2Layout>
      <Switch>
        <Route path="/ui2" component={UI2Home} />
        <Route path="/ui2/search" component={UI2Search} />
        <Route path="/ui2/service/:id" component={UI2Service} />
        <Route path="/ui2/book/:id" component={UI2BookService} />
        <Route path="/ui2/profile" component={UI2Profile} />
        <Route path="/ui2/my-bookings" component={UI2MyBookings} />
        <Route path="/ui2/chat" component={UI2Chat} />
        <Route path="/ui2/favorites" component={UI2Favorites} />
        <Route path="/ui2/notifications" component={UI2Notifications} />
        <Route path="/ui2/user/:id" component={UI2UserProfile} />
        <Route path="/ui2/referrals" component={UI2Referrals} />
      </Switch>
    </UI2Layout>
  );
}
